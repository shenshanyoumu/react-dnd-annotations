...
export default class HTML5Backend implements Backend {
  ...
  // 取消注册事件，并且结束AnimationFrame机制的周期性监听
  public teardown() {
    if (this.window === undefined) {
      return;
    }
  ...
  // 下面这种编程形式在redux中也用到，比如store的subscribe()返回unscribe()函数
  ...
  // 得到当前拖拽过程中的呈现效果，比如源DOM节点以“move“形式的移动；或者”copy“形式被复制移动
  ...
  private getCurrentSourcePreviewNodeOptions() {
    const sourceId = this.monitor.getSourceId() as string;
    const sourcePreviewNodeOptions = this.sourcePreviewNodeOptions.get(
      sourceId
    );
  ...
  // 当DOM节点在拖拽过程中，由于被从DOM结构中删除则直接结束本轮拖拽
  ...
  // 处理被拖拽的DOM结构中最外层的拖拽行为，因为可以一次拖拽多个嵌套的DOM节点
  ...
  // 当拖放事件发生，则将目标DOM节点的ID添加到内部维持的dropTargetIds数组头部
